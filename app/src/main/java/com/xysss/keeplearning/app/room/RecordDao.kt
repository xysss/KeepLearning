...
interface RecordDao {
    ...
    //使用内连接查询
    @Query("select id,timestamp,cf,alarm,userId,placeId,matterName,ppm from Record left join Matter on Record.voc_index_record=Matter.voc_index_matter order by id desc limit :size offset :index")
